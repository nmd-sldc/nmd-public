...
set infile [open flash:dev_list.txt]; 
  ...
  if {[file exists $config_fq_filename]==1} {
    switch $device_type {
      "AccessSwitch" {
          ...
          close $temp_file
          puts "Applying baseline configuration to device"
          exec "config replace flash:/ccnp/base/base.cfg force"
          puts "Applying trouble ticket to running configuration"
          typeahead "\r";exec "copy flash:/temp_file.cfg running-config"
          puts "Deleting temporary files. Please press Enter twice."
          typeahead "\r";exec "delete flash:/temp_file.cfg"
          puts "\r\n"
          puts "#########################################"
          puts "#### Device reconfiguration complete ####"
          puts "#########################################"
